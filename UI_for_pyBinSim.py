        ...
        def Body_tracker(stop_event):
            ...
            try:
                while not stop_event.is_set():
                    ...
                    # roll_out_rs = min(availableAngles_yaw, key=lambda x: abs(x - roll_current_rs))
                    yaw_out_rs = yaw_current_rs

                    # roll_out_rs  = roll_out_rs*-1 ########################## POV : You are at the front of the camera #########################
                ...
                pipe.stop()   ### remove the comment when using real sense
                # pass # comment it when using real sense
...
######################################################################################################################################################
        ...
        def Close(): # function to close application and binsim
            global stop_loop
            stop_loop = True
...
### can be used for testing purpose #############################
        ...
        # def next_room(): # function to change to next room
        ...
        def create_circle():# function to create a circle with angles for head tracking
